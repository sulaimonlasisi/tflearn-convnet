...
'''
files_path = './symposium_prints/'

good_files_path = os.path.join(files_path, 'split_good_magenta', 'good*.png')
streak_files_path = os.path.join(files_path, 'split_magenta_streaks',  'streak*.png')

good_files = sorted(glob(good_files_path))
streak_files = sorted(glob(streak_files_path))

n_files = len(good_files) + len(streak_files)
print(n_files)
...
# test-train split   
X, X_test, Y, Y_test = train_test_split(allX, ally, test_size=0.2, random_state=5)
...
# Wrap the network in a model object
model = tflearn.DNN(network, tensorboard_verbose=0, checkpoint_path = 'split-prints-cnn.tfl.ckpt', max_checkpoints = 2, best_val_accuracy = 0.9)
...
model.fit(X, Y, n_epoch=100, shuffle=True, validation_set=(X_test, Y_test),
          ...
          snapshot_epoch=True,
          run_id='split-prints-cnn')
print("Done with training")

# Save model when training is complete to a file
model.save("split-prints-cnn.tfl")
print("Network trained and saved as split-prints-cnn.tfl!")
''' 